...
//     // const userEventsData = {"evenements": []};
//     const [userEventsData, getUserEvents] = useState(null);

//     useEffect(() => {
//         axios.get('http://localhost:3001/myplanning/userEvents.json')
//             .then(response => getUserEvents(response.data))
...
//     const getAllUsersEvents = () => {
//         const allUsersEvents = [];
//         userEventsData.evenements.forEach(event => {
//             const userData = { username: event.username }; // Créer une structure de données pour stocker les événements par utilisateur
//             const events = filterUserEvents(userData);
//             allUsersEvents.push({ username: event.username, events: events });
//         });
...
//                 if (!currentUrl.includes("recap?index=")) {
...
//         const allUsersEvents = getAllUsersEvents(); // Récupérer les événements pour tous les utilisateurs

//         const newHighlightedCells = [];

//         allUsersEvents.forEach(user => {
...
const RecapPage = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3001/myplanning/userEvents.json')
            .then(response => setUserEventsData(response.data))
            .catch(error => console.error('Error fetching data:', error));
    }, []);
    ...
    useEffect(() => {
        if (userEventsData) {
            ...
            const getAllUsersEvents = () => {
                const allUsersEvents = [];
                userEventsData.evenements.forEach(event => {
                    const userData = { username: event.username };
                    const events = filterUserEvents(userData);
                    allUsersEvents.push({ username: event.username, events: events });
                });
                return allUsersEvents;
            ...
            const allUsersEvents = getAllUsersEvents();
